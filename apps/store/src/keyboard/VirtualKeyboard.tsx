...
/** Teclado no ecrã (QWERTY/numérico) para terminais táteis. Ver apps/pos. */
export function VirtualKeyboard({ layout, submitLabel = 'OK', onInsert, onBackspace, onClear, onSubmit, onHide }: Props) {
      ...
      <div className="kbd">
        ...
        {NUMERIC_ROWS.map((row) => (
          <div className="kbd-row" key={row.join()}>
            {row.map((ch) => (<button type="button" className="key" key={ch} onMouseDown={press(() => onInsert(ch))}>{ch}</button>))}
          </div>
        ))}
        ...
        <div className="kbd-row"><button type="button" className="key accent" onMouseDown={press(onSubmit)}>{submitLabel}</button></div>
      </div>
    );
  }
  ...
  return (
    <div className="kbd">
      {Bar}
      <div className="kbd-row">{rows[0].map((ch) => (<button type="button" className="key" key={ch} onMouseDown={press(() => onInsert(disp(ch)))}>{disp(ch)}</button>))}</div>
      <div className="kbd-row">{rows[1].map((ch) => (<button type="button" className="key" key={ch} onMouseDown={press(() => onInsert(disp(ch)))}>{disp(ch)}</button>))}</div>
      <div className="kbd-row">
        {mode === 'letters' ? (
          <button type="button" className={`key wide-15 ${shift ? 'accent' : 'action'}`} onMouseDown={press(() => setShift((s) => !s))}><IconShift size={20} /></button>
        ) : (<span className="key wide-15" style={{ visibility: 'hidden' }} />)}
        {rows[2].map((ch) => (<button type="button" className="key" key={ch} onMouseDown={press(() => onInsert(disp(ch)))}>{disp(ch)}</button>))}
        <button type="button" className="key action wide-15" onMouseDown={press(onBackspace)}><IconBackspace size={22} /></button>
      </div>
      <div className="kbd-row">
        <button type="button" className="key action wide-16" onMouseDown={press(() => setMode((m) => (m === 'letters' ? 'symbols' : 'letters')))}>{mode === 'letters' ? '?123' : 'ABC'}</button>
        <button type="button" className="key" onMouseDown={press(() => onInsert(','))}>,</button>
