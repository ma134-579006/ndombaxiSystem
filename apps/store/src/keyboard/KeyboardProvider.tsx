...
const LS_KEY = 'ndombaxi.store.virtualKeyboard';

/** UM teclado no ecrã partilhado (terminais táteis). Ver apps/pos. */
export function KeyboardProvider({ children }: { children: React.ReactNode }) {
  ...
  const open = useCallback((field: ActiveField) => { cancelTimer(); setActive(field); }, []);
  const close = useCallback(() => { cancelTimer(); setActive(null); }, []);
  ...
  const value = useMemo<KeyboardContextValue>(
    () => ({ enabled, setEnabled, toggle: () => setEnabled(!enabled), activeId: active?.id ?? null, open, close, scheduleClose }),
